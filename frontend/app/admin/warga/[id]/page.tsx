...
export default function AdminWargaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetchWithAuth(`/rt/warga/${id}`)
  ...
  return <WargaDetail warga={warga} backHref="/admin/warga" />
}
