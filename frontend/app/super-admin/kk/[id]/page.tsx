...
export default function SuperAdminKKDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const [kk, setKK] = useState<KartuKeluarga | null>(null)
  const [loading, setLoading] = useState(true)

  const fetchData = async () => {
    const response = await fetchWithAuth(`/rw/kartu-keluarga/${id}`)
  ...
  return <KKDetail kk={kk} backHref="/super-admin/kk" wargaHrefBase="/super-admin/warga" onChanged={fetchData} />
}
